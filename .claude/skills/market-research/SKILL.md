...
2. Run the full research method below. Mine the real world, not just the files: use web search and page fetches to pull genuine voice-of-customer (review pages, Reddit threads, forums, articles) and link every source. For sources the plain web tools cannot reach, the tool order is (see `SETUP.md`): **Scrapling first** (Reddit at depth, review sites that block bots - it is free; use its StealthyFetcher), **Apify where Scrapling can't go** (X, TikTok, and Instagram comments), and **Firecrawl when an ordinary page just will not load** (JavaScript-heavy sites, review pages, PDFs - it is free and keyless, but it refuses Reddit and cannot reach logged-in social, so it replaces neither of the other two). Use whatever is set up; if a tool is missing, note in the document which platforms went unmined.
