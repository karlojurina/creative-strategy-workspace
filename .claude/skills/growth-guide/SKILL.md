...
0. **Open with the explanation, then wait.** Tell the user what the growth guide is before wiring it: the heartbeat of the brand - the Idea Bank (every raw idea, dated, nothing gated) plus the Concepts board that tracks each ad from concept to briefing to editing to testing to its learning and verdict. It is one markdown file, `growth-guide.md`, and the visual app reads and writes that same file. This step fills it with the brand's avatars and desires so every concept card can name the one avatar and one desire it is built on. Ask if they're ready.
