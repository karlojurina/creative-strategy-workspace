...
1. Read `MANIFESTO.md`. Ask which brand and which concept card if it is not obvious. Then load the full picture: the brand's `CONTEXT.md`, `research/market-research.md`, `research/avatars.md`, `research/desires.md`, `learnings/learnings.md`, the concept's card in `growth-guide.md`, and `resources/breakthrough-advertising/`. Past learnings shape this script - that is the compounding. Skipping this loading step is how generic AI copy happens; it is non-negotiable.
