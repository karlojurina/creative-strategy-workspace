...
| `growth-guide-app/` | The visual growth guide (kanban, idea wall, results, analytics, and the Library tab that renders `resources/` for reading while you work). Start it with `node growth-guide-app/server.js` - it reads and writes the same markdown files, so the app and the files never disagree. |
